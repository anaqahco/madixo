...
const LOCATION_ONLY_INPUTS = new Set([
  'السعودية',
...
const GENERIC_IDEA_PATTERNS = [
  /اريد\s+ان\s+اصبح\s+تاجر/,
  /أريد\s+أن\s+أصبح\s+تاجر/,
  /ابي\s+اصير\s+تاجر/,
  /ابغى\s+اصير\s+تاجر/,
  /اريد\s+مشروع/,
  /أريد\s+مشروع/,
...
const DIRECT_SALE_IDEA_PATTERNS = [
  /^(?:بيع|متجر|براند|ماركة|تصنيع|توزيع|توريد|استيراد)\s+[؀-ۿa-zA-Z0-9].{3,}$/i,
  /^(?:sell|selling|store|brand|manufacturing|distribution|supply|import|importing)\s+[a-zA-Z0-9].{3,}$/i,
];

const CUSTOMER_ROLE_WORDS = [
  'مالك',
  'صاحب',
  'اصحاب',
  'أصحاب',
...
function countWordsForInputValidation(value: string) {
...
function looksLikeLocationOnlyInput(value: string) {
  return LOCATION_ONLY_INPUTS.has(normalizeComparableText(value));
}

function hasAnyKeyword(value: string, keywords: string[]) {
  const simplified = normalizeComparableText(value).replace(/[^a-z؀-ۿ0-9\s]/g, '').trim();
  return keywords.some((keyword) => simplified.includes(keyword));
}
...
export async function POST(request: Request) {
  try {
    ...
    const notSpecifiedFallback =
      outputLanguage === 'ar' ? 'غير محدد' : 'Not specified';
