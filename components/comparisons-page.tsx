...
import { localizeText, type ComparisonPage } from '@/lib/blog';
import { useUiLanguageState } from '@/components/ui-language-provider';

export default function ComparisonsPageClient({ items }: { items: ComparisonPage[] }) {
  ...
  return (
    <>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-6 pb-24 md:grid-cols-2 xl:grid-cols-2">
        {items.map((item) => (
          <article key={item.slug} className="rounded-[28px] border border-[#D9E2F0] bg-[#F7F9FC] p-6 shadow-sm">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href={`/compare-to/${item.slug}`} className="rounded-full bg-[#111827] px-5 py-3 text-sm font-semibold text-white hover:opacity-90">
                {copy.open}
              </Link>
